...
module clint(
    ...
    reg[`INST_ADDR_BUS]         instr_addr;
    reg[`INST_ADDR_BUS]         div_instr_addr;
    
    // 如果是除法指令，则保存除法指令的地址
    always @ (posedge clk or negedge rst_n) begin
        if(!rst_n) begin
            div_instr_addr <= `ZERO_WORD;
        end
        else if (instr_i == `INS_DIV || instr_i == `INS_DIVU || instr_i == `INS_REM || instr_i == `INS_REMU) begin
            div_instr_addr <= instr_addr_i;
        end
        else begin
            div_instr_addr <= div_instr_addr;
        end
    end
    ...
    always @ (*) begin
        ...
        // 同步中断
        //if (instr_i == `INS_ECALL || instr_i == `INS_EBREAK || (instr_i[6:0] == `INS_TYPE_CSR && privileg_i < `PRIVILEG_MACHINE)) begin
        else begin
            if (instr_i == `INS_ECALL || instr_i == `INS_EBREAK) begin
            ...
            case (csr_state)
                CSR_IDLE: begin
                    ...
                    if (int_state == INT_SYNC_ASSERT) begin
                        ...
                        //if (instr_i == `INS_ECALL && privileg_i == `PRIVILEG_MACHINE) begin
                        if (instr_i == `INS_ECALL) begin
                            cause <= 32'd11;
                        end
                        // Environment call from U-mode
                        //else if (instr_i == `INS_ECALL && privileg_i == `PRIVILEG_USER) begin
                        //    cause <= 32'd8;
                        //end
                        // Breakpoint
                        ...
                        // Illegal Instruction
                        //else if (instr_i[6:0] == `INS_TYPE_CSR && privileg_i < `PRIVILEG_MACHINE) begin
                        //    cause <= 32'd2;
                        //end
                        else begin
                            cause <= 32'd10;
